...
RBF 之后又扩了 25 个形态，用**纯自然语言输入**在多个宿主（aider / gptme / codex）上验证 skill 的通用性与边界。加速比列 codex 版（诚实 baseline 下的独立复验值，代表 skill 能达到的水平）。下表 28 行 = 26 个形态 + 2 个加固衍生 case（`temperature_softmax`/`l2norm_scale`，softmax/归一化家族内为验证确认闸门与反向融合手段而生，不单列形态序号）：
...
| integral_image | 2D前缀和/积分图(scan升2D) | 4.90 / 5.97 | 稳赢(前向需并行/流水优化;三宿主一致达标) |
| warp_sparse_contrast | warp级稀疏加权聚合+双梯度 | 4.15 / 10.90 | 稳赢(warp shfl规约省中间物化) |
...
已验证的 case（**26 形态**，覆盖归约/矩阵乘/scan/卷积/位置编码/数据依赖写/间接寻址/点积/分组归约/逐元素融合链/空间采样/变长分段/线性求解/稠密分解/2D数据依赖stencil/2D前缀和/warp级稀疏聚合等维度，framework 零改动即支持每一个）：`rbf` · `layernorm` · `softmax_ce` · `scan` · `gemm_bias_gelu` · `online_softmax`（§12 有实测，未 collect 成 case 目录）· `rope` · `linear_ssm` · `welford` · `causal_attn` · `conv1d` · `gated_ssm` · `scatter_add` · `topk` · `maxpool` · `spmv` · `cosine_sim` · `temperature_softmax` · `l2norm_scale` · `groupnorm` · `geglu` · `gridsample` · `segment_softmax` · `tridiag` · `cholesky` · `dynamic_grid_evolution` · `integral_image` · `warp_sparse_contrast`。逐形态对照与结论见 [`skill/AGENT_TEST_MATRIX.md`](skill/AGENT_TEST_MATRIX.md)。
