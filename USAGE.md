...
```
对一批多通道的时间序列做逐通道的因果一维卷积：每个通道用它自己的一个短卷积核，沿时间
方向滑动加权求和，且只依赖当前和过去的时间点、不看未来。数据是 float32，对输入序列和
卷积核都要能求梯度。
```
...
<然后用自然语言描述你的算法>
例如："对一批多通道时间序列做逐通道的因果一维卷积，每个通道用自己的短卷积核沿时间滑动加权，只依赖当前和过去、不看未来，fp32，对输入和卷积核都求梯度。"
```
