...
- ✅ **达标**：前向、反向加速比均 ≥1.05×，且 3 次重跑 CV≤5%，正确性仍全 PASS。→ 进入交付：生成 `cases/<name>/delivery/`（独立编译版）并 `make test` 跑 CPU 对拍打印 PASS。**完工 = 加速比达标 且 delivery `make test` PASS**（见 SKILL.md 步骤 7/达标判据）。
