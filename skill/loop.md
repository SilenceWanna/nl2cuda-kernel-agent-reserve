...
- ✅ **达标或认定本征边界**：前反向加速比均 ≥1.05×（3 次重跑 CV≤5%、正确性全 PASS）**，或** 正确性全 PASS + 反向达标/已尽力 + 前向多规模复测坐实是本征边界（带宽墙/厂商库墙）。→ 进入交付：生成 `cases/<name>/delivery/`（独立编译版）并 `make test` 跑 CPU 对拍打印 PASS；本征边界形态 README 加"性能边界说明"。**完工 = 正确性全 PASS 且 delivery `make test` PASS 且（性能达标 或 已诚实报本征边界）**（见 SKILL.md 步骤 7/达标判据）。
...
## 过度优化翻车教训（优化非越多越好——每步测收益，无效/变慢立即回退）
...
- **不挑规模凑擦线（规模挑选=短核假象变种，禁）**：擦线未过时，正解是**优化 kernel 本体**，不是从多个规模里挑一个恰好能擦过的短核规模（固定开销撑高加速比）。固定一个计算主导区规模（baseline ≥1ms）优化到达标。`run_on_a100.sh` 会对擦线 PASS+短核自动 ×2/×4 复测，掉破 1.05 判 `PASS_SCALE_SUSPECT`（strict 下未达标）——挑更大规模也过不了，只能真优化。见 SKILL.md 达标判据"规模挑选"。—— 实测：GroupNorm gptme 挑 GN_N=384(baseline 前0.92ms)前向擦线1.07，放大768/1536 掉1.048/1.035 被拆穿。
