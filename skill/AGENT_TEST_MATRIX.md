...
**二十二形态光谱更新**：稳赢区新增 **tridiag（串行依赖线性系统求解）**——前向批量 Thomas 略赢 PCR，**反向大幅赢（解析伴随系统 vs autograd 机械反传 PCR）**。**元规律扩展——"CUDA 能利用数学结构、autograd 不能"成为一类稳赢来源**：gated_ssm/linear_ssm（稳定 O(T/N) 递推/前缀）、tridiag（线性求解反向=解伴随系统）——这类算子 candidate 不是"访存更少"而是"**用了 autograd 不知道的解析结构**"，反向常有数量级优势。确认闸门加固经 codex 两个点算子名 case（grid_sample/Thomas）稳定守住。
