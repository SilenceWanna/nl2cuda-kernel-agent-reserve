...
### 步骤 7：交付（闭环必做，达标或认边界即自动生成，不必等用户额外提示）
**达标（`VERDICT=PASS`）或诚实认定本征边界后**（本征边界=前向带宽墙/厂商库墙：正确性全 PASS、反向达标或已尽力、前向多规模复测坐实打不过），照 `cases/rbf/delivery/` 结构在 `cases/<name>/delivery/` 手写四文件，产出**不依赖 PyTorch、可独立 `nvcc` 编译**的交付版：
...
然后 `cd cases/<name>/delivery && make test` 独立编译跑 CPU 对拍，**须打印 PASS**。**完工判据 = 正确性全 PASS 且 delivery `make test` PASS 且（性能达标 或 已诚实报本征边界）**。厂商库墙/带宽墙等"诚实报边界"形态也须生成 delivery（手写尽力版 + README 加"性能边界说明"、不宣称超 torch.compile），其 `make test` 正确性仍须 PASS。
...
> **元规律:反向常比前向好赢**——前向多是搬运(带宽墙),反向常含 Jacobian/伴随系统/多超越函数/列规约等**解析结构或计算**,candidate 能省 torch.compile 机械反传的多趟物化。**连整行归一化反向也能赢——只要把多个梯度(dX/dgamma/dbeta)融进单个 kernel、输入各只读一遍**(访存约为拆分多 kernel 的一半,翻过带宽墙;实测 LayerNorm 反向单 kernel 全融合 1.9×,而拆分两 kernel 版撞墙 0.99)。但**别默认"反向总能赢"**——若反向也拆成多 kernel 各读一遍输入,低算术强度归一化反向同样撞带宽墙,**赢在融合结构、不在算子本身**。
> （各区间的**具体 case + 实测加速比**见 [CASE_EVIDENCE.md](CASE_EVIDENCE.md)，三宿主测试时剥离。）

> **识别本征边界——何时该停（避免在打不赢的算子上空转轮次）**：
>
> **⚙️ 未达标先三分归因（先判是哪一类，再决定行动，别盲目烧轮次）**：某侧（前向或反向）计算主导区达不到 1.05× 时，先归类——
> - **① 算法本征边界（打不过是对的，正解=诚实报边界，别再烧轮次）**：a) **纯访存前向带宽墙**——candidate 与 baseline 访存量相同且都近峰值带宽（2×2 池化读4写1、逐元素、小 reduce 归一化前向）；b) **厂商库墙前向**——baseline 就是 NVIDIA 厂商成品（cuSOLVER 分解/求解、cuBLAS 大 GEMM、cuFFT），手写分块极难企及。**判据**：三宿主一致卡同一点、且放大到计算主导区仍打不过。→ 认边界、诚实记录，不是失败。
> - **② 宿主实现力不足（skill 指对了方向，但当前 agent 写不出高效 kernel）**：**同一 case 有宿主赢、有宿主挂**（如 attention/scatter/topk/GEMM+尾融合：codex 达标，中弱宿主挂；RMSNorm 反向：codex 全融合 1.9× / aider 拆分擦线 / gptme 并行写崩 0.29×）。→ 这不是 skill 缺方法、也不是本征边界，是 agent 的 kernel 编码力差距。**能做的**：把 skill 里对应手段（单 kernel 全融合、float4、避 atomic 竞争、shared tiling）更明确地指给它；换更强宿主往往就过。
> - **③ skill 指导短板（方向本身没讲清，连最强宿主也没达标）**：若**连 codex 类强宿主、方向也对，却仍差一截**，才可能是 skill 缺了某类算子的优化通法——此时该把新手法补进 skill 的优化决策表。**实测现状：几乎不存在纯③类**——强宿主能达标的形态 skill 都已覆盖；剩下打不过的基本是①（本征）或②（宿主力）。**别把①②误判成③去空补 skill，也别把③的真空白当①认输。**
>
> **判定要点**：先问"三宿主是否一致卡同一点"——一致卡=①本征；有赢有挂=②宿主力；都对方向却都差=③（罕见，才补 skill）。以下是①带宽墙的细化判据：
...
>   - **大 reduce/点积（规约元素 ~万级 或 点积/exp 规约）→ 算术强度够，可真赢**。**判据**：看单输出规约元素数/算术强度——~1K 逐元素→带宽墙、~万级或点积/exp→可真赢。**实测锚点**：softmax_ce（exp 规约）计算主导区前 1.33~1.84×/反 1.26~1.39× 前反向都真赢，GroupNorm（大分组 reduce）前 1.28~1.40×——**别把"有规约"一律当带宽墙，exp/大 reduce 前向本就赢**。
>   - **反向不一定更好赢，但常可赢**：大 reduce/点积类反向有 Jacobian/列规约/缓存复用可优化（真赢）；整行归一化反向**若多梯度融进单 kernel、输入各只读一遍也能翻过带宽墙真赢**（实测 LayerNorm 反向单 kernel 全融合 1.9×）；只有拆成多 kernel 各读一遍输入时才撞墙。**反向能否赢看融合结构 + 算术强度，别默认"反向总能/总不能赢"**。
...
> - **关键告诫："能优化"（结构上）≠"能赢 torch.compile"（计算主导区）**：归一化**前向**常有 float4+寄存器缓存等正确优化，但**性能达标可能仍是短核假象**——放大到计算主导区掉破 1.05 即带宽墙本征边界（前向坐实打不过）。**反向**则要看 kernel 结构：缓存 mean/rstd 消 O(B·D²) 重算是正确性必修，而**能否赢取决于是否把多梯度融进单 kernel 各只读一遍输入**（融合真赢 1.9×、拆分撞墙）。**判达标一律计算主导区多规模验证**（换 2×/4× 规模：真优势稳、短核假象掉）。
...
- **独立编译交付版（闭环必做）**：达标**或诚实认定本征边界**后须生成 `cases/<name>/delivery/`（见步骤 7）并 `make test` 独立编译（仅 nvcc + CUDA runtime、无 torch）跑 CPU 对拍打印 PASS。
- **完工 = 正确性全 PASS + delivery `make test` PASS +（性能达标 或 已诚实报本征边界）**。本征边界形态（前向带宽墙/厂商库墙）不因前向 <1.05× 而卡住交付——正确性 + 反向尽力 + delivery + README 注边界即完工。
