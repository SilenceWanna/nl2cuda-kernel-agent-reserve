...
> - **③ skill 指导短板（方向本身没讲清，连最强宿主也没达标）**：若**连 codex 类强宿主、方向也对，却仍差一截**，才可能是 skill 缺了某类算子的优化通法——此时该补 skill（记入 CASE_EVIDENCE + 决策表）。**实测现状：几乎不存在纯③类**——强宿主能达标的形态 skill 都已覆盖；剩下打不过的基本是①（本征）或②（宿主力）。**别把①②误判成③去空补 skill，也别把③的真空白当①认输。**
>
