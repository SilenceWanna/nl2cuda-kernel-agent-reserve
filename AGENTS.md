...
3. **写实现**（严格按 SKILL.md "Case 协议"的 7 字段和骨架）：
   ...
   - `__init__.py`：组装 `CASE`（7 字段）。**直接照 `cases/rbf/__init__.py` 用 `Case(name=…, description=…, params=…, grad_inputs=…, dtype=…, make_inputs=…, reference_forward=…)` 显式传全部字段**——**别用 `inspect.signature` 反射 / 字段白名单自造构造**（实测某宿主自造反射构造漏了 `params`/`grad_inputs`/`dtype` 必填字段 → `unsupported required field 'params'` 卡住）。最简可靠就是照 rbf 样例直接实例化，字段定义见 `framework/case.py`。
